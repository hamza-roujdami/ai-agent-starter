...
1. Run the test suite in `tests/` (including `tests/test_eval.py`). Show the output.
2. If a Foundry project is configured, run a batch evaluation using the **microsoft-foundry** skill
   (quality/groundedness/relevance graders) against a small dataset curated from `tests/`.
